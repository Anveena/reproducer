...
static BIO *create_socket_bio(const char *hostname, const char *port, int family, BIO_ADDR **peer_addr) {
     ...
     */
    if (!BIO_lookup_ex(hostname, port, BIO_LOOKUP_CLIENT, family, SOCK_DGRAM, 0, &res)) return NULL;
...
static void wait_for_activity(SSL *ssl) {
     ...
     * both)
     */
    if (SSL_net_write_desired(ssl)) FD_SET(sock, &wfds);
    if (SSL_net_read_desired(ssl)) FD_SET(sock, &rfds);
    width = sock + 1;
     ...
     */
    if (SSL_get_event_timeout(ssl, &tv, &isinfinite) && !isinfinite) tvp = &tv;
 ...
 */
void *newConn(void *arg) {
    const ConnInfo *info = (ConnInfo *)arg;
    SSL *ssl = NULL;
    BIO *bio = NULL;
    int res = EXIT_FAILURE;
    int ret;
    unsigned char alpn[] = {13, 'a', 'n', 'A', 'l', 'p', 'n', 'F', 'o', 'r', 'T', 'e', 's', 't'};
    unsigned char *toSendData = (unsigned char *)malloc(sizeof(uint32_t) * 2);
    uint32_t *toSend = (uint32_t *)toSendData;
    toSend[0] = info->generation;
    toSend[1] = info->total;
    size_t written, readbytes = 0;
    char buf[256];
    BIO_ADDR *peer_addr = NULL;
    int eof = 0;
    const char *hostname = SERVICE_HOST;
    const char *port = SERVICE_PORT;
     ...
     * connection.
     */
    bio = create_socket_bio(hostname, port, AF_INET, &peer_addr);
    if (bio == NULL) {
    ...
    while ((ret = SSL_connect(ssl)) != 1) {
        if (handle_io_failure(ssl, ret) == 1) continue; /* Retry */
        printf("Failed to connect to server\n");
        goto end; /* Cannot retry: error */
    }

    while (!SSL_write_ex(ssl, toSendData, sizeof(uint32_t) * 2, &written)) {
        if (handle_io_failure(ssl, 0) == 1) continue; /* Retry */
    ...
    do {
         ...
         */
        if (!eof) fwrite(buf, 1, readbytes, stdout);
    } while (!eof);
    ...
    while ((ret = SSL_shutdown(ssl)) != 1) {
        if (ret < 0 && handle_io_failure(ssl, ret) == 1) continue; /* Retry */
    }
     ...
     * information there.
     */
    if (res == EXIT_FAILURE) ERR_print_errors_fp(stderr);
...
int main(void) {
    ...
    uint32_t generation = time(NULL);
    pthread_t threads[THREAD_COUNT];
    void **infoArray = malloc(sizeof(void *) * THREAD_COUNT);
    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
        ConnInfo *info = malloc(sizeof(ConnInfo));
        info->ctx = ctx;
        info->generation = generation;
        info->total = THREAD_COUNT;
        pthread_t thread;
        pthread_create(&thread, NULL, newConn, (void *)info);
        threads[i] = thread;
        infoArray[i] = info;
    }
    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
